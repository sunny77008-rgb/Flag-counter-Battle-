...
async function fetchComments() {
  ...
  if (dailyQuotaUsed[currentKeyIndex] > DAILY_LIMIT) {
  ...
  try {
    ...
    dailyQuotaUsed[currentKeyIndex] += 5;
    ...
    res.data.items.forEach(msg => {
      ...
      if (countryCode) {
        ...
        if (!currentTarget && isRoundActive && !isGameOver) {
          ...
          console.log(`🎯 TARGET: ${countryMap[countryCode]} by ${username}`);
        }
      }
    });
...
io.on("connection", (socket) => {
  ...
  if (currentTarget && isRoundActive) {
    socket.emit("setTarget", {
      countryCode: currentTarget,
      username: "Arena",
      countryName: countryMap[currentTarget]
    });
  }
